...
// target is a cylinder, inner radius 0 cm, radius 0.5cm and length 0.0001 cm. Adjust target size to raise statistics
    G4Tubs* target_tube = new G4Tubs("target", 0.0*cm, 0.90*cm, 0.0001*cm, 0.0*deg, 360.0*deg);
...
//........DIMENSIONS of target holder.........
// target is a rectangle, 2.5 cm wide and 1.80 cm tall until you get to the circle cut out.
    G4Box* targetHolder_box = new G4Box("targetHolder",2.5*cm, 1.60*cm, 0.1*cm);
    //target_holder is made of aluminum and is called target_holder
    G4LogicalVolume* targetHolder_log = new G4LogicalVolume(targetHolder_box,Al,"targetHolder",0,0,0);
    // place it in the center of the coordinate system
    new G4PVPlacement(0,G4ThreeVector(0*cm,-2.5*cm,0*cm),targetHolder_log,"targetHolder",room_log,false,0);


//........DIMENSIONS of target around.........
// target is a cyllinder, inner radius .5 cm, radius 1.25 cm and length .1 cm until you get to the circle cut out.

    G4Tubs* targetCircle_tube = new G4Tubs("targetCircle", 0.90*cm, 1.25*cm, 0.1*cm, 0.0*deg, 360.0*deg);
    //target is full of gold and is called 'target'
    G4LogicalVolume* targetCircle_log = new G4LogicalVolume(targetCircle_tube,Al,"targetCircle",0,0,0);
    // place it in the center of the coordinate system
    new G4PVPlacement(0,G4ThreeVector(0*cm,0*cm,0*cm),targetCircle_log,"targetCircle",room_log,false,0);
