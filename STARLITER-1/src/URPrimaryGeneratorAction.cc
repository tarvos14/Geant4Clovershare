...
G4double ExcEnergy=.429*MeV;
 ...
 G4double px, py, pz, costheta, sintheta, phi, sci;       // CoM frame
 G4double x, y, z;
 G4double xAve=0;
 G4double yAve=0;
	...
	//basically, need to decide angle from 0 to .85, then decide which direction it goes. px=sintheta*std::cos(phi), py sin(phi)
	phi=twopi*G4UniformRand();
	sci=twopi*G4UniformRand();

    px = 1.0*sintheta*std::cos(phi);
    py = 1.0*sintheta*std::sin(phi);
    pz = 1.0*costheta;
...
*/
	...
	//Sign assign
/*
	G4double SignAssign =0.5-G4UniformRand();
	if(SignAssign>0)
	{
		SignAssign=1;
	}
...
*/
	...
	//Need donut ring for 6Li. Centered at 0, densist around 10, extends from 15 to 5.

	double Width=BeamWidth;
	double Offset=BeamOffset;

	//grid search  width from 2 to 8 mm  and offset from 3.0 to 7.0 mm
	G4double beamPosX = Offset*mm;
	G4double beamPosY = 0.0*mm;
	G4double beamPosR = 0.0*mm;
	G4double xUncert = (Width*2)*mm;
	G4double yUncert = (Width*2)*mm;
	G4double rUncert = Width*mm;
	//G4double sci=twopi*G4UniformRand();
	//G4double xDisplacement = 10*G4UniformRand()*SignAssign*mm;
	//G4double RadiusDisplacement=10*mm;
	...
	while((x-beamPosX)*(x-beamPosX)+(y-beamPosY)*(y-beamPosY)>rUncert*rUncert)
	{
		x=beamPosX+xUncert*(0.5-G4UniformRand());
		y=beamPosY+yUncert*(0.5-G4UniformRand());
		Reroll++;
...
/*
	...
	while(abs(r)>rUncert)
	{
		//G4cout<< r<< " " << rUncert<< " "<<Reroll<< G4endl;
		r=gRandom->Gaus(beamPosR, rUncert)*mm;
		//Reroll++;
